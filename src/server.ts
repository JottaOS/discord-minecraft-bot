...
const restartServer = async () => {
	if (!mcProcess) return utils.log({ prefix: '[BOT]', message: 'El servidor se intentó reiniciar pero no estaba en ejecución.', color: 'Yellow', important: true });
	utils.log({
		prefix: '[BOT]',
...
const stop = async (message: OmitPartialGroupDMChannel<Message>) => {
	...
	response.edit({
		embeds: [utils.embed({ description: `Servidor detenido en \`${timer()}\`.`, color: 'Red' })],
	});
};
