...
const startServer = async () => {
	if (!mcProcess) {
		...
		if (!opened) {
			utils.log({
				prefix: '[BOT]',
				message: 'Se ha agotado el tiempo de espera para iniciar el servidor.',
				color: 'Red',
				important: true,
			});
...
const start = async (message: OmitPartialGroupDMChannel<Message>) => {
	if (busy) return await message.reply({
		allowedMentions: { repliedUser: false },
		embeds: [
			utils.embed({
				description: 'Espera a que se ejecute por completo el comando anterior.',
				color: 'Yellow',
			}),
		],
	});
